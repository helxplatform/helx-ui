...
export const PaginationTray = () => {
  ...
  return (
    <Wrapper role="navigation" aria-label="Pagination Navigation">
      <PaginationLink to={ `/helx/workspaces/search?q=${ query }&p=1` } icon="firstPage" disabled={ currentPage <= 1 } aria-label="Go to first page" />
      <PaginationLink to={ `/helx/workspaces/search?q=${ query }&p=${ currentPage - 1 }` } icon="chevronLeft" disabled={ currentPage <= 1 } aria-label="Go to previous page"/>
      <Icon icon="ellipsis" fill={ currentPage > paginationRadius + 1 ? theme.color.grey.light : 'transparent' } size={ 24 } />
      {
        [...Array(pageCount).keys()].map(i => {
          ...
          return (
            <PaginationLink
              key={ `link-to-p${ i + 1 }` }
              to={ `/helx/workspaces/search?q=${ query }&p=${ i + 1 }` }
      ...
      <PaginationLink to={ `/helx/workspaces/search?q=${ query }&p=${ currentPage + 1 }` } icon="chevronRight" disabled={ currentPage >= pageCount } aria-label="Go to next page"/>
      <PaginationLink to={ `/helx/workspaces/search?q=${ query }&p=${ pageCount }` } icon="lastPage" disabled={ currentPage >= pageCount } aria-label="Go to last page"/>
    </Wrapper>
  )
}
